...
console.log(obj == true); // true (hint: "default")

// Date 对象特殊处理：优先 toString
const d = new Date(0);
console.log(d == 0);      // false (d.toString() → "Thu Jan 01 1970..." → NaN)
console.log(+d);          // 0     (hint: "number", d.valueOf() → 0)
```
...
class ManagedResource {
  ...
  allocate(id) {
    const handle = { id, buffer: new ArrayBuffer(1024) };
    this.#registry.register(handle, id);
    return handle;
  }
}

const manager = new ManagedResource();
let res = manager.allocate('file-1');
res = null; // 取消引用，未来某个时刻触发清理回调
```
...
// Atomics 等待/通知模式（生产者-消费者）
// Worker A (消费者): 等待信号
Atomics.wait(array, 0, 0); // 阻塞直到 array[0] !== 0

// Worker B (生产者): 发送信号
Atomics.store(array, 0, 1);
Atomics.notify(array, 0, 1); // 唤醒 1 个等待者
```
...
// eval 的严格模式差异
function sloppyEval() {
  eval('var x = 10'); // x 泄漏到 sloppy 函数作用域
}

function strictEval() {
  'use strict';
  eval('var y = 20'); // y 被限制在 eval 自己的作用域内
...
function outerFixed() {
  const local = 42;
  const fn = new Function('local', 'return local + 1');
  return fn(local); // 43
}
```
...
// 修复跨 Realm instanceof 的实用工具
function safeInstanceof(obj, Ctor) {
  // 方法1: 使用 Symbol.hasInstance（如果可用）
  if (Ctor && typeof Ctor[Symbol.hasInstance] === 'function') {
    return Reflect.apply(Ctor[Symbol.hasInstance], Ctor, [obj]);
  }
  // 方法2: 回退到构造函数比较
  return obj != null && (
    obj.constructor === Ctor ||
    Object.prototype.toString.call(obj) === Object.prototype.toString.call(Ctor.prototype)
  );
}
```
...
- [ECMA-262 §7.2.14 Abstract Equality Comparison](https://tc39.es/ecma262/#sec-abstract-equality-comparison)
...
- [ECMA-262 §9.1 Ordinary Object Internal Methods](https://tc39.es/ecma262/#sec-ordinary-object-internal-methods-and-internal-slots)
- [ECMA-262 §9.10 WeakRef Objects](https://tc39.es/ecma262/#sec-weak-ref-objects)
- [ECMA-262 §27.4 Atomics Object](https://tc39.es/ecma262/#sec-atomics-object)
- [ECMA-262 §19.2.1 eval(x)](https://tc39.es/ecma262/#sec-eval-x)
- [V8 博客: Orinoco GC](https://v8.dev/blog/orinoco)
- [V8 博客: Trash talk](https://v8.dev/blog/trash-talk) — V8 垃圾回收详解
...
- [MDN: instanceof and cross-realm](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/instanceof#instanceof_and_multiple_realms)
- [2ality: Abstract equality comparison](https://2ality.com/2011/06/sameness-in-equality.html) — Dr. Axel Rauschmayer
- [2ality: typeof null](https://2ality.com/2013/10/typeof-null.html) — 历史原因深度解析
