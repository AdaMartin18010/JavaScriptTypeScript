...
export const searchUsers = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: query }) => {
    ...
    const { results } = await env.DB.prepare(
      'SELECT id, name FROM users WHERE name LIKE ? LIMIT 20'
    ).bind(`%${query}%`).all();
    return results;
  });
...
function UserSearch() {
  ...
  const handleSearch = async (q: string) => {
    const users = await searchUsers({ data: q });
    setResults(users);
  };
}
```
...
const UserSearchSchema = z.object({
  tab: z.enum(['profile', 'settings', 'billing']).default('profile'),
  edit: z.coerce.boolean().default(false),
});

export const Route = createFileRoute('/users/$userId')({
  // 路径参数类型：{ userId: string }
  component: UserProfilePage,
  loader: async ({ params, location }) => {
    // params.userId 已类型化为 string
    const { tab } = UserSearchSchema.parse(location.search);
...
- [TanStack Start GitHub Discussions](https://github.com/TanStack/router/discussions)
- [TanStack Router API Reference](https://tanstack.com/router/latest/docs/framework/react/api/router)
...
- [Remix vs TanStack Start — Tanner Linsley](https://www.youtube.com/watch?v=4L1z-6cT1Ck)
