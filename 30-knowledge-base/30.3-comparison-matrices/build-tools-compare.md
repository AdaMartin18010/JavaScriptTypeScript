<!--
# 合并说明
- 合并来源: docs/comparison-matrices/build-tools-compare.md（完整版，185 行）、30-knowledge-base/30.3-comparison-matrices/build-tools-compare.md（精简版，46 行）
...
|------|------|---------|--------|
| **冷启动 (1000模块)** | ~300ms (Vite 6) / ~1.7s (Vite 5) | ~5.6s | ~2s |
| **热更新时间** | ~20ms (deep component) | 500ms–1.6s | ~100ms |
| **生产构建** | 快 (Rolldown) | 中 | 快 |
| **内存占用** | 低 | 高 | 低 |
| **产物大小** | 小 | 中 | 最小 |
...
| 指标 | Babel (JS) | SWC (Rust) | esbuild (Go) | 倍数差距 |
|------|-----------|------------|--------------|---------|
| **转译 1000 个 TS 文件** | ~12s | **~0.8s** | ~0.6s | SWC **15×**  faster |
| **Minify 大型代码库** | ~8s | **~0.5s** | ~0.4s | SWC **16×** faster |
| **Watch 模式增量编译** | ~2.5s | **~0.15s** | ~0.12s | SWC **17×** faster |
| **多线程利用** | 单线程 | 全核并行 | 全核并行 | — |
| **TS 类型剥离** | 需插件 | 内置 | 内置 | — |

*数据来源：SWC 官方基准 2025、BetterStack 独立测试、社区 Benchmark 2026。*

## Vite 6 升级前后构建对比（真实项目实测）

| 项目规模 | Vite 5 (Rollup) | Vite 6 (Rolldown 实验性) | 构建时间降幅 |
|---------|----------------|-------------------------|-------------|
| 小型项目 (100 模块) | ~0.8s | **~0.3s** | **62%** |
| 中型项目 (500 模块) | ~4.2s | **~1.5s** | **64%** |
| 大型项目 (2000 模块) | ~18s | **~6s** | **67%** |
| 超大型项目 (生产 + minify) | ~28s | **~10s** | **64%** |

*数据来源：Vite 团队 Rolldown-Vite 技术预览报告（2025–2026），包含 GitLab、Excalidraw、Appwrite 等真实项目数据。*

### 独立第三方基准（2026-03，800 组件/140 路由/95K 行 TS 真实项目，M3 MacBook Pro）

| 指标 | Rspack 1.x | Turbopack (Next.js 15) | Vite 8 (Rolldown) |
|------|-----------|----------------------|-------------------|
| **冷启动** | **1.4s** | 2.1s | 2.8s |
| **HMR (深层组件)** | 160ms | **70ms** | 130ms |
| **生产构建** | **3.4s** | N/A (Next.js build: 8.2s) | 4.1s |
| **Webpack 配置复用** | ~95% 兼容 | 0% (新配置) | 0% (新配置) |

*来源：Kunal Ganglani 独立基准测试 [kunalganglani.com](https://www.kunalganglani.com/blog/vite-turbopack-rspack-benchmark)，2026-03-26。*
...
Rolldown 是由 Vite 团队（VoidZero，Evan You 创立）基于 Rust + Oxc 打造的下一代打包器：

- **当前状态**：**1.0 Stable 已于 2026 年 4 月发布**，Vite 8.0+ 默认生产打包引擎
- **性能**：比 Rollup/Webpack 快 **4 倍–18 倍**，大型项目构建时间减少 38%–64%
- **真实案例**：GitLab 迁移到 Rolldown-Vite 后，构建从 **2.5 分钟 → 22 秒**（**7x 提升**），相比原始 Webpack 构建 **43x 更快**
- **独立 CLI**：已推出，不依赖 Vite 也可直接使用；API 与 Rollup 兼容
- **核心优势**：单一 Rust 引擎替代 esbuild + Rollup 双引擎，消除 dev/prod 行为差异，Tree Shaking 与代码分割由 Rolldown 原生完成

### Turbopack（2026 状态）

Turbopack 是 Vercel 为 Next.js 打造的 Rust 原生增量打包器，由 Webpack 原作者 Tobias Koppers 主导开发：

- **当前状态**：Next.js 15+ 默认开发服务器，生产构建已于 2025 年底达到 **Stable**（`areweturboyet.com` 显示 100% 测试通过）
- **性能**：大型 Next.js 项目冷启动从 Webpack 的 **18s → 0.8s**（23× 提升）；HMR 从 **1.2s → 20ms**（60× 提升）；生产构建从 **142s → 38s**（3.7× 提升）
- **独立基准验证**：800 组件真实项目，Turbopack HMR **70ms**（Vite 8: 130ms, Rspack: 160ms）；冷启动 2.1s（Rspack 1.4s, Vite 8 2.8s）
- **限制**：仅限 Next.js 生态，无独立 CLI；插件生态远小于 Vite；生产构建产物体积比 Webpack **大 72%**（首屏 JS）
- **2026 展望**：Vercel 持续优化持久化缓存与 monorepo 支持，暂无跨框架计划
