...
| **Cold Start** | :star::star::star: 快 (无二进制) | :star::star::star: 极快 | :star::star::star: 快 | :star::star: 中 | :star::star: 中 |
| **Edge Support** | :white_check_mark: Native | :white_check_mark: Native | :white_check_mark: Native | :x: | :x: |
...
| **Access Control** | :x: 需自行实现 | :x: 需自行实现 | :white_check_mark: `@@allow` / `@@deny` | :x: 需自行实现 | :x: 需自行实现 |
| **维护状态** | :green_circle: 活跃 | :green_circle: 活跃 | :green_circle: 活跃 | :yellow_circle: 维护模式 | :yellow_circle: 维护模式 |
...
> :warning: **维护模式声明**：截至 2026年，TypeORM 和 Sequelize 核心团队活动显著减少，主要进入 bug fix 与社区 PR 维护阶段。新项目不推荐选用。
...
| **Access Control** | :x: | :x: | :white_check_mark: 声明式 | :x: | :x: |
...
|------|----------|-------------|----------|---------|-----------|
| **自动生成类型** | :white_check_mark: 完整 | :white_check_mark: 完整（从 TS Schema） | :white_check_mark: 完整 | :warning: 部分 | :warning: 部分 |
| **查询返回值类型** | :star::star::star: 精确 | :star::star::star: 精确 | :star::star::star: 精确 | :star::star: 良好 | :star::star: 良好 |
| **关系类型** | :star::star::star: 自动 | :star::star::star: 自动 | :star::star::star: 自动 | :star::star: 需配置 | :star::star: 需配置 |
| **部分选择类型** | :star::star::star: 支持 | :star::star::star: 支持 | :star::star::star: 支持 | :star::star: 需手动 | :star::star: 需手动 |
| **select * 类型推导** | :star::star::star: 支持 | :star::star::star: 支持 | :star::star::star: 支持 | :star::star: 部分 | :star::star: 部分 |
...
| **冷启动** | 快 (无二进制) | 极快 | 快 | 中 | 中 |
| **边缘运行** | :white_check_mark: Native | :white_check_mark: Native | :white_check_mark: Native | :x: | :x: |
...
## 代码示例：Drizzle ORM RLS 策略映射

```typescript
// drizzle-rls.ts -- Drizzle ORM 实现 PostgreSQL Row Level Security
import { pgTable, serial, varchar, integer, pgPolicy } from 'drizzle-orm/pg-core';
...
export const documents = pgTable('documents', {
  ...
  // 定义 RLS 策略：用户只能访问自己租户的数据
  tenantIsolationPolicy: pgPolicy('tenant_isolation', {
    for: 'all',
    to: 'public',
    using: sql`${table.tenantId} = current_setting('app.current_tenant')::integer`,
    withCheck: sql`${table.tenantId} = current_setting('app.current_tenant')::integer`,
  }),
}));
...
// 使用示例
const result = await withTenantContext(db, 'tenant-42', async () => {
  return db.select().from(documents).where(eq(documents.ownerId, 1));
});
```

## 代码示例：Prisma 加速查询与连接池优化

```typescript
// prisma-optimize.ts -- Prisma 7 连接池与查询优化实践
...
// 批量操作优化：使用 createMany 替代循环 create
async function bulkInsertUsers(users: { email: string; name?: string }[]) {
  return prisma.user.createMany({
    data: users,
    skipDuplicates: true, // 自动忽略唯一约束冲突
  });
}

// 查询优化：使用 select 精简字段，避免 N+1
async function getUserSummaries() {
  return prisma.user.findMany({
    select: { id: true, email: true, name: true }, // 仅返回必要字段
    take: 100,
...
async function transferFunds(fromId: number, toId: number, amount: number) {
  return prisma.$transaction(async (tx) => {
    ...
    isolationLevel: 'Serializable', // 高隔离级别防止并发异常
    maxWait: 5000,
    timeout: 10000,
  });
}

// 边缘缓存：结合 Prisma Accelerate（可选）
// import { withAccelerate } from '@prisma/extension-accelerate';
// const acceleratedPrisma = prisma.$extends(withAccelerate({ cacheStrategy: { ttl: 60 } }));
```
...
> :calendar: 本文档最后更新：2026-04
>
> :bulb: 提示：Stars 数据会随时间变化，建议查看 GitHub 获取最新数据；Prisma 7 为 2025.11 发布的新版本，生态正在快速演进。
...
| 资源 | 链接 | 说明 |
|------|------|------|
| Prisma 官方文档 | [prisma.io/docs](https://www.prisma.io/docs) | ORM 与数据库工具 |
| Drizzle ORM 官方文档 | [orm.drizzle.team](https://orm.drizzle.team) | TypeScript ORM |
| ZenStack 官方文档 | [zenstack.dev](https://zenstack.dev) | Prisma 访问控制扩展 |
| Turso / libSQL 文档 | [turso.tech/docs](https://docs.turso.tech) | 边缘 SQLite |
| Neon 官方文档 | [neon.tech/docs](https://neon.tech/docs) | Serverless Postgres |
| Cloudflare D1 文档 | [developers.cloudflare.com/d1](https://developers.cloudflare.com/d1) | 边缘数据库 |
| PlanetScale 文档 | [planetscale.com/docs](https://planetscale.com/docs) | Vitess MySQL |
| MikroORM 文档 | [mikro-orm.io](https://mikro-orm.io) | TypeScript ORM (Unit of Work) |
| TypeORM 官方文档 | [typeorm.io](https://typeorm.io) | 装饰器驱动 ORM |
| Sequelize 官方文档 | [sequelize.org](https://sequelize.org) | 老牌 Node.js ORM |
| PostgreSQL 文档 | [postgresql.org/docs](https://www.postgresql.org/docs/) | 关系数据库 |
| SQLite 文档 | [sqlite.org/docs.html](https://www.sqlite.org/docs.html) | 嵌入式数据库 |
| Prisma 7 WASM 公告 | [prisma.io/blog/prisma-7](https://www.prisma.io/blog) | WASM 引擎迁移 |
| Drizzle Kit 迁移指南 | [orm.drizzle.team/kit-docs](https://orm.drizzle.team/kit-docs/overview) | 迁移工具 |
| Zod 官方文档 | [zod.dev](https://zod.dev) | Schema 验证 |
| Valibot 官方文档 | [valibot.dev](https://valibot.dev) | 轻量验证库 |
| Neon Branching 文档 | [neon.tech/docs/introduction/branching](https://neon.tech/docs/introduction/branching) | Git 式数据库分支 |

> :calendar: 本节补充更新：2026-04-27
