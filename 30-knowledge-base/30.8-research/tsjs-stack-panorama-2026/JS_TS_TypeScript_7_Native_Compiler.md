...
## 4.4 开发者迁移准备：tsconfig 最佳实践

为了在 TypeScript 7.0 到来时实现平滑迁移，团队应从今日开始采用以下配置策略：

```json
// tsconfig.json —— 面向 TypeScript 7.0 的迁移准备配置
{
  "compilerOptions": {
    "target": "ES2022",
    "module": "NodeNext",
    ...
    // ✅ 启用 erasableSyntaxOnly（Node.js Type Stripping 兼容）
    "erasableSyntaxOnly": true,
    // ❌ 避免使用 enum，改用对象字面量 + as const
    // ❌ 避免使用 namespace，改用 ES Module
    // ❌ 避免使用 parameter properties（非可擦除语法）
    "strict": true,
...
// 等价于：type Direction = 0 | 1 | 2 | 3
```

> **关键原则**：坚持使用 TypeScript 的*可擦除语法*（erasable syntax），确保代码可以在 TypeScript 7.0 原生编译器和 Node.js Type Stripping 模式下无缝运行。
...
9. TypeScript Team. *TypeScript 7.0 Native Compiler (Corsa) Preview*. <https://github.com/microsoft/TypeScript/issues/61142>
10. Go Team. *The Go Memory Model*. <https://go.dev/ref/mem>
11. Node.js Documentation. *TypeScript Type Stripping*. <https://nodejs.org/api/typescript.html#type-stripping>
12. ECMA International. *ECMA-262 16th Edition (ES2025)*. <https://tc39.es/ecma262/>

## 进阶示例：Go 原生编译器 CI 集成与基准测试

```yaml
# .github/workflows/typecheck.yml
name: Type Check (Native + JS)
...
jobs:
  ...
  tsc-native:
    ...
    steps:
      - uses: actions/checkout@v4
      - name: Download TS 7.0 Preview
        run: |
          curl -L -o tsc-go https://github.com/microsoft/TypeScript/releases/download/v7.0.0-preview/tsc-linux-amd64
          chmod +x tsc-go
      - run: ./tsc-go --noEmit
```

```bash
# benchmark.sh — 对比 JS tsc 与 Go tsc 的构建时间
echo "JS tsc:"
time npx tsc --noEmit
...
```typescript
// server.ts — Node.js Type Stripping 与 erasableSyntaxOnly 实践
...
- [Microsoft TypeScript Blog — A New Era for TypeScript](https://devblogs.microsoft.com/typescript/a-new-era-for-typescript/) — 官方公告原文
- [TypeScript 7.0 / Corsa Preview Issue](https://github.com/microsoft/TypeScript/issues/61142) — GitHub 追踪 Issue
- [Node.js Type Stripping Documentation](https://nodejs.org/api/typescript.html#type-stripping) — Node.js 官方类型剥离文档
- [Go Programming Language](https://go.dev/) — Go 语言官方网站
- [Language Server Protocol Specification](https://microsoft.github.io/language-server-protocol/) — LSP 标准规范
- [SWC Project](https://swc.rs/) — 基于 Rust 的高速 transpiler
- [esbuild Documentation](https://esbuild.github.io/) — 极速 bundler 官方文档
- [TypeScript 5.8 Release Notes](https://devblogs.microsoft.com/typescript/announcing-typescript-5-8/) — 5.8 版本官方发布说明
- [Go Memory Model](https://go.dev/ref/mem) — Go 内存模型参考
- [TC39 ECMA-262 Specification](https://tc39.es/ecma262/) — ECMAScript 语言规范
- [VS Code Repository](https://github.com/microsoft/vscode) — 微软 VS Code 开源仓库
- [Node.js Performance Best Practices](https://nodejs.org/en/docs/guides/simple-profiling) — Node.js 官方性能指南
