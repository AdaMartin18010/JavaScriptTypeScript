...
# 生成 TurboFan IR 图（需要 d8 或 node 调试构建）
node --print-opt-code --code-comments app.js

# 使用 Chrome DevTools Protocol 采集性能数据
node --inspect-brk app.js
# 然后使用 Chrome DevTools 或 Puppeteer 连接
```

```javascript
// 检测函数是否被 V8 优化（用于性能分析）
function isOptimized(fn) {
  return %GetOptimizationStatus(fn) & 16; // 需要 --allow-natives-syntax
}

// 生产环境安全的启发式检测
function detectDeoptimization() {
  ...
  // 如果 duration 异常高，可能发生了 Deoptimization
}
```
...
```typescript
// 使用 Performance API 测量渲染性能
function measureRenderMetrics() {
  // INP (Interaction to Next Paint)
  const observer = new PerformanceObserver((list) => {
    for (const entry of list.getEntries()) {
      if (entry.entryType === 'event' && entry.duration > 200) {
        console.warn(`Slow interaction: ${entry.name}, duration: ${entry.duration}ms`);
      }
    }
  });
...
- [V8 Blog](https://v8.dev/blog) — Google V8 团队官方技术博客
- [V8 Design Docs](https://v8.dev/docs) — V8 架构与内部设计文档
- [TypeScript Specification](https://github.com/microsoft/TypeScript/blob/main/doc/spec.md) — TypeScript 语言规范
- [ECMA-262 Specification](https://tc39.es/ecma262/) — JavaScript 语言标准
- [Node.js Release Notes](https://nodejs.org/en/blog/release/) — Node.js 版本发布说明
- [Bun Blog](https://bun.sh/blog) — Bun 运行时官方博客
- [Deno Blog](https://deno.com/blog) — Deno 运行时官方博客
- [web.dev Rendering Performance](https://web.dev/articles/rendering-performance) — Google 渲染性能权威指南
- [Chrome DevRel Web Vitals](https://web.dev/articles/vitals) — Web Vitals 核心指标
- [React Server Components RFC](https://github.com/reactjs/rfcs/blob/main/text/0188-server-components.md) — RSC 架构 RFC
- [Google Project Zero](https://googleprojectzero.blogspot.com/) — 高级漏洞研究
- [NVD — National Vulnerability Database](https://nvd.nist.gov/) — CVE 权威数据库
- [MITRE CVE](https://cve.mitre.org/) — 通用漏洞枚举
- [State of JS Survey](https://stateofjs.com/) — JavaScript 生态年度调查
- [Gradual Typing — Siek & Taha](https://wphomes.soic.indiana.edu/jsiek/what-is-gradual-typing/) — 渐变类型学术基础
- [Chromium Security](https://www.chromium.org/Home/chromium-security/) — Chromium 安全策略
- [WebAssembly Spec](https://webassembly.github.io/spec/core/) — WASM 核心规范
- [WebGPU Explainer](https://gpuweb.github.io/gpuweb/explainer/) — WebGPU 技术说明
- [MDN Web Docs](https://developer.mozilla.org/) — Web 技术权威参考
- [Can I Use](https://caniuse.com/) — 浏览器特性兼容性矩阵
