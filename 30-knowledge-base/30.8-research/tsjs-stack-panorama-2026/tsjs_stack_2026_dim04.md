...
```typescript
// server.ts — Deno 显式权限声明
// 运行：deno run --allow-net=:8000 --allow-read=./data server.ts

import { serve } from "https://deno.land/std@0.224.0/http/server.ts";

// 默认零权限：任何未声明的 API 调用将抛出 PermissionDenied
async function handler(req: Request): Promise<Response> {
  const url = new URL(req.url);

  // 需要 --allow-read 权限
  if (url.pathname === "/config") {
    const config = await Deno.readTextFile("./data/config.json");
...
#### 代码示例：Node.js 权限模型（Node.js 20+ Experimental Permission Model）

```javascript
// permission-check.mjs — Node.js 实验性权限 API
import { permission } from "node:process";

// 检查文件系统权限
const readPermission = permission.has("fs.read", "./sensitive-data.json");
console.log("Can read sensitive data:", readPermission); // false (unless --allow-fs-read)
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    messages,
    // 工具调用：Agent 可自主决定调用外部函数
    tools: {
      getTypeDocs: {
        description: "Get TypeScript type documentation",
        parameters: z.object({ typeName: z.string() }),
        execute: async ({ typeName }) => {
          return await fetchTypeDocs(typeName);
        },
      },
    },
  });
...
#### 代码示例：MCP TypeScript SDK 工具服务器

```typescript
// mcp-server.ts — Model Context Protocol 工具定义
...
const server = new Server(
  { name: "ts-docs-server", version: "1.0.0" },
  {
    capabilities: {
      tools: {
        lookupType: {
          description: "Look up TypeScript type definition",
          inputSchema: {
            type: "object",
            properties: {
...
server.setRequestHandler("tools/call", async (request) => {
  if (request.params.name === "lookupType") {
    const docs = await lookupTypeDocs(request.params.arguments.typeName);
...
#### 代码示例：Chrome Built-in AI API（Native Browser AI）

```javascript
// 使用 Chrome 内置 AI 进行文本摘要（需 Chrome 128+ 并启用实验性标志）
async function summarizeText(text) {
  // 检查 API 可用性
  if (!('ai' in self) || !self.ai.summarizer) {
    throw new Error('Native AI API not available');
  }
...
// 使用
const longArticle = "..."; // 长文本
summarizeText(longArticle).then(console.log);
```
