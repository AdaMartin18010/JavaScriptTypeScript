...
- **关联文件/模块**：
  ...
  - `10-fundamentals/10.1-language-semantics/control-flow/`
- **关键技能**：
  - 词法作用域 vs 动态作用域
...
- **关联文件/模块**：
  - `20-code-lab/20.1-language-core/js-ts-comparison/`
...
- **关键技能**：
  ...
  - 结构类型系统 vs 名义类型系统
  - TypeScript 装饰器元数据（Stage 3 Decorators + `accessor`）
...
- **关键技能**：
  ...
  - V8 引擎的 Ignition + TurboFan + Maglev（中间层编译器）流水线
...
- **关键技能**：
  ...
  - 条件导出（`import`/`require`/`default`/`types`）的最佳实践
...
- **关联文件/模块**：
  ...
  - `20-code-lab/20.11-rust-toolchain/metaprogramming/`
...
- **难度**：⭐⭐ | **预计时间**：1 周
- **扩展**：使用 TypeScript 5.6 `NoInfer<T>` 优化事件名称推断
...
- **难度**：⭐⭐⭐⭐⭐ | **预计时间**：3–4 周
- **扩展选项 C（Rust 工具链）**：使用 SWC 或 Oxc 插件 API 实现一个自定义 JS/TS 代码转换器
  - **代码位置**：`20-code-lab/20.11-rust-toolchain/`
  - **通过标准**：能正确转换指定语法模式，通过集成测试
...
- [TC39 Proposals Tracker](https://tc39.es/proposals/) — 实时追踪 Stage 0–3 提案
- [TypeScript 5.6 Release Notes](https://devblogs.microsoft.com/typescript/announcing-typescript-5-6/) — 最新语言特性
- *Types and Programming Languages* — Benjamin C. Pierce
- *Compilers: Principles, Techniques, and Tools* (Dragon Book)
- [TLA+ Video Course](https://lamport.azurewebsites.net/video/videos.html)
- `10-fundamentals/` — 语言核心与类型系统深度理论（新架构）
