...
export async function gatewayRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  // 服务发现状态（调试用）
  fastify.get("/discovery/services", async () => {
  ...
  fastify.get("/users/profile", { onRequest: [requireAuth] }, async (request: FastifyRequest) => ({
    user: request.user,
  }));
  ...
  fastify.route({
    ...
    handler: async (
      ...
      const result = await proxyRequest("users-service", path, request.method, {
  ...
  fastify.route({
    ...
    handler: async (
      ...
      const path = `/${request.params["*"]}`;
      const result = await proxyRequest("orders-service", path, request.method, {
        authorization: request.headers.authorization,
        "x-request-id": request.id,
      }, request.body as Record<string, unknown> | undefined);
      return reply.status(result.statusCode).headers(result.headers).send(result.body);
  ...
  fastify.get("/", async () => ({
    service: "api-gateway",
    version: "1.0.0",
    features: ["dynamic-service-discovery", "rate-limit", "jwt-auth"],
