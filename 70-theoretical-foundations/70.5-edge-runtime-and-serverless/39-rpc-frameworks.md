...
- [RPC 框架与类型安全传输](#rpc-框架与类型安全传输)
  ...
  - [3. Connect RPC：Protobuf 与 HTTP/2 的融合](#3-connect-rpcprotobuf-与-http2-的融合)
    - [3.1 Connect 的三协议支持](#31-connect-的三协议支持)
    - [3.2 Protobuf 与 TypeScript 的绑定](#32-protobuf-与-typescript-的绑定)
  - [4. gRPC-Web：从数据中心到浏览器](#4-grpc-web从数据中心到浏览器)
    - [4.1 gRPC-Web 的代理需求](#41-grpc-web-的代理需求)
    - [4.2 流式调用的限制](#42-流式调用的限制)
  - [5. JSON-RPC 2.0：轻量级替代方案](#5-json-rpc-20轻量级替代方案)
    - [5.1 协议语义](#51-协议语义)
    - [5.2 与 tRPC/Connect 的对比](#52-与-trpcconnect-的对比)
  - [6. Schema 演化与版本兼容性](#6-schema-演化与版本兼容性)
    - [6.1 Protobuf 的向后兼容性规则](#61-protobuf-的向后兼容性规则)
    - [6.2 TypeScript 类型的演化挑战](#62-typescript-类型的演化挑战)
  - [7. Edge 场景下的 RPC 优化](#7-edge-场景下的-rpc-优化)
    - [7.1 Edge 函数的 RPC 调用模式](#71-edge-函数的-rpc-调用模式)
    - [7.2 流式与订阅的边缘化](#72-流式与订阅的边缘化)
  - [8. 范畴论语义：RPC 作为远程态射](#8-范畴论语义rpc-作为远程态射)
  - [9. 对称差分析：REST vs RPC vs GraphQL](#9-对称差分析rest-vs-rpc-vs-graphql)
  - [10. 工程决策矩阵](#10-工程决策矩阵)
  - [11. 反例与局限性](#11-反例与局限性)
    - [11.1 tRPC 的版本锁定反例](#111-trpc-的版本锁定反例)
    - [11.2 Protobuf 的 JSON 互操作陷阱](#112-protobuf-的-json-互操作陷阱)
    - [11.3 gRPC-Web 的双向流缺失](#113-grpc-web-的双向流缺失)
    - [11.4 RPC 的"本地调用幻觉"](#114-rpc-的本地调用幻觉)
...
tRPC 默认使用 **HTTP/1.1 JSON**，但支持多种传输层：
...
## 9. 对称差分析：REST vs RPC vs GraphQL
...
| 场景 | 推荐方案 | 理由 | 风险 |
|------|---------|------|------|
| 全栈 TypeScript 单体应用 | tRPC | 端到端类型安全，零代码生成，Zod 运行时验证 | 与外部系统（非 TS）集成困难 |
...
| Edge 函数间通信 | Connect Protocol (JSON) | 纯 HTTP/1.1，无原生依赖 | 性能低于 protobuf 二进制 |
