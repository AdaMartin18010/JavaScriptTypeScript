...
这对 JavaScript 性能模型的影响是深远的。首先，"编译速度"与"峰值性能"之间的传统权衡将被重新定义：开发者不再需要在"快速启动"和"极致热点优化"之间做显式取舍，因为统一的编译器可以根据调用频次和类型稳定性动态选择优化深度。其次，CFG 的线性结构使得去优化（deoptimization）路径更加可预测，减少了因图调度不确定性导致的性能抖动。最后，更短的编译管线意味着更小的内存占用和更快的代码缓存恢复速度，这对容器化、Serverless 等冷启动敏感场景尤为重要。
...
| `nodenext` | Node.js 22+ | 目前行为等同于 `node16`，但映射到 Node.js 22+ 的解析规则 [Juejin: TS moduleResolution 精讲] |
...
#### 4.6.3 `fs.watch` AsyncIterator 优化

Node.js 24 对文件系统监控 API 进行了重要的异步迭代器重构。`fs.watch` 及其递归变体现在返回一个 **AsyncIterator**，允许开发者使用 `for await...of` 以声明式方式消费文件系统事件，而无需手动管理 `EventEmitter` 的监听器注册与内存泄漏风险 [Node.js 24 Release Notes]：

```js
import { watch } from 'node:fs';

for await (const event of watch('./src', { recursive: true })) {
  console.log(`类型: ${event.eventType}, 文件: ${event.filename}`);
}
```

这一改动不仅是 API 表面的优雅化，更深层的影响在于**背压（backpressure）控制**：AsyncIterator 的 `next()` 调用天然具备流量控制能力，当消费者处理速度落后于内核 `inotify`/`ReadDirectoryChangesW` 的事件产生速度时，事件队列不会无限膨胀。这与传统 `EventEmitter` 的"fire-and-forget"模型形成鲜明对比，显著提升了高变更频率场景下的内存稳定性。
...
### 5.6 WebAssembly 前沿：WasmGC 与 Type Canonicalization
...
## 6. 运行时三足鼎立：Node.js vs Deno 2 vs Bun 1.3

### 6.1 格局概述：Node.js 24 vs Deno 2.x vs Bun 1.3

2025-2026 年，JavaScript/TypeScript 运行时形成了三足鼎立的稳定格局：

| 维度 | Node.js 24 | Deno 2.x | Bun 1.3 |
|------|-----------|----------|---------|
| **设计哲学** | 生态成熟度优先 | 安全 + Web 标准优先 | 极致性能优先 |
| **运行时引擎** | V8 | V8 | JavaScriptCore (JSC) |
| **语言实现** | C++ | Rust | Zig |
| **npm 兼容性** | 原生 | 完全兼容 (`npm:` 前缀) | 98% |
| **TypeScript 支持** | `--strip-types` 默认启用（仅剥离） | 原生零配置 | 原生零配置 |
| **内置工具链** | 无（需额外安装） | fmt/lint/test/bundle | 包管理器 + 测试 + 运行 |
| **安全模型** | 无沙箱 | 权限沙箱 (`--allow-*`) | 无沙箱 |
| **HTTP 吞吐量** | 30k req/s | 40k req/s | **68k req/s** |
| **冷启动** | ~100ms | ~50ms | **8-15ms** |
| **包安装速度** | 基准 | 2x | **10-50x** |
| **Windows 支持** | 完美 | 良好 | **稳定** |
| **边缘部署** | 有限 | Deno Deploy | Cloudflare Workers |

### 6.2 Node.js 22/23/24+：生态之王

Node.js 仍然是企业级应用的首选运行时，其核心竞争力在于**生态成熟度**。

**关键进展（2025-2026）**：

1. **`--experimental-strip-types` 默认启用**
   - Node.js 22 起，可直接运行 `.ts` 文件（仅剥离类型，不转译）
   - 与 `tsx` 的区别：Node.js 的 strip-types 仅支持 erasable syntax（如 `type`、接口），不支持 `enum`、`namespace`
   - **意义**：Node.js 官方首次承认 TypeScript 的合法地位

2. **`require(ESM)` 稳定**
   - Node.js 22 起，`require()` 可直接加载 ESM 模块
   - 解决了 CJS/ESM 互操作的最后一块拼图
...
4. **V8 Maglev 升级**
   - Node.js 23+ 搭载 V8 12.x，Maglev 编译器覆盖率提升
   - 温热代码性能提升 8-15%
...
### 6.3 Deno 2：安全与标准的守护者

Deno 2（2024 年 10 月发布）是 Deno 的成熟版本，核心定位是**安全优先 + Web 标准对齐**。

**关键特性**：

1. **完全 npm 兼容**
   - `npm:` 前缀可直接使用 npm 包
   - `deno install` 安装 npm 依赖
   - 解决了 Deno 1.x 的最大痛点（生态隔离）
...
- 安全敏感的应用（金融、医疗）
- 边缘计算（Deno Deploy）
- 需要 Web 标准对齐的项目（Fetch API、Web Streams）
- 希望减少工具链复杂度的团队

### 6.4 Bun 1.3：性能怪兽

Bun 1.3（2026 年初发布）将性能推向了新的高度，核心定位是**极致性能 + 一体化工具链**。

**关键数据**：

- HTTP 吞吐量：**68k req/s**（Node.js 的 2.3 倍）
- 包安装：1847 依赖仅需 **47 秒**（npm 需要 28 分钟）
- 冷启动：**8-15ms**（Node.js 的 ~100ms）
- 内存占用：比 Node.js 低 **10-30%**
- npm 兼容性：**98%**

**关键特性**：

1. **一体化工具链**
   - `bun` = 包管理器 + 运行时 + 测试运行器 + 打包器
   - 单个工具替代 `npm` + `node` + `jest` + `webpack`

2. **JavaScriptCore (JSC) 引擎**
   - 选择 JSC 而非 V8 的原因：启动速度更快、内存占用更低
   - Safari/WebKit 使用的同一引擎
...
4. **原生 S3 文件系统**
   - `Bun.file("s3://bucket/object")` 直接读取 S3
   - 边缘场景（Cloudflare Workers）中极为实用

5. **Windows 稳定**
   - Bun 1.3 的 Windows 支持达到生产级
   - 消除了早期版本的平台限制

**适用场景**：

- 高性能 API 服务
- 需要极速启动的 Serverless 场景
- 新建项目（无历史包袱）
- 对构建速度极度敏感的大型 Monorepo
...
| 评估维度 | Node.js 24 | Deno 2 | Bun 1.3 |
|---------|-----------|--------|---------|
| 生态成熟度 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| 运行时性能 | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| 启动速度 | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
...
- **Bun 1.3** 在性能维度全面领先，适合新建高性能项目
- **Deno 2** 在安全性和 Web 标准维度领先，适合边缘计算和安全敏感场景
- **Node.js 24** 在生态和企业支持维度无可替代，适合企业级项目

### 6.6 迁移路径建议

**从 Node.js 迁移到 Bun**：

1. 运行 `bun install` 测试依赖兼容性
2. 运行测试套件，修复不兼容的测试
3. 替换 `package.json` 中的 scripts（`node` → `bun`，`npm test` → `bun test`）
4. CI 中并行运行 Node.js 和 Bun，逐步切换

**从 Node.js 迁移到 Deno**：

1. 添加 `npm:` 前缀到所有 npm 导入
2. 将 CommonJS 转换为 ESM
3. 配置权限（`--allow-read`、`--allow-net`）
4. 使用 `deno.json` 替代 `package.json`
