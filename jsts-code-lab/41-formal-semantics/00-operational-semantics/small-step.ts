...
export type Config = { expr: Expr; store: Store }
  ...
  switch (expr.kind) {
    ...
    case 'add': {
      if (!isValue(expr.left)) {
        const next = step({ expr: expr.left, store });
        if (next.kind === 'val') return step({ expr: { kind: 'add', left: { kind: 'num', value: next.value }, right: expr.right }, store: next.store });
        return { expr: { kind: 'add', left: next.expr, right: expr.right }, store: next.store };
      }
      if (!isValue(expr.right)) {
        const next = step({ expr: expr.right, store });
        if (next.kind === 'val') return step({ expr: { kind: 'add', left: expr.left, right: { kind: 'num', value: next.value } }, store: next.store });
        return { expr: { kind: 'add', left: expr.left, right: next.expr }, store: next.store };
      }
    ...
    case 'seq': {
      if (!isValue(expr.first)) {
        const next = step({ expr: expr.first, store });
        if (next.kind === 'val') return step({ expr: expr.second, store: next.store });
        return { expr: { kind: 'seq', first: next.expr, second: expr.second }, store: next.store };
      }
      return step({ expr: expr.second, store });
    }
  }
}

export function evalSmallStep(expr: Expr, store: Store = new Map()): { value: number; store: Store } {
  let config: Config | { kind: 'val'; value: number; store: Store } = { expr, store };
  let steps = 0;
