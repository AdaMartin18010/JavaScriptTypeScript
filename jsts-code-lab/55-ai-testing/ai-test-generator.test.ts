...
describe('AITestGenerator', () => {
  it('generates tests for a two-parameter function', () => {
    ...
    expect(tests.some((t: any) => t.name.includes('valid'))).toBe(true);
    expect(tests.some((t: any) => t.name.includes('boundary'))).toBe(true);
    expect(tests.some((t: any) => t.name.includes('invalid'))).toBe(true);
  });

  it('generates valid input based on type', () => {
    const gen = new AITestGenerator();
    const tests = gen.generateTests('function greet(name: string): string { return name; }', 'greet');
    const valid = tests.find((t: any) => t.name.includes('valid'));
