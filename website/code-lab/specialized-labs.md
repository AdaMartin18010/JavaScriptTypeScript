...
| 你的背景 | 推荐起点 | 进阶方向 |
|----------|----------|----------|
| 前端新手 | `lab-01-basic-setup` → `language-core-lab` | `svelte-signals-lab` |
| 有 React/Vue 经验 | `async-concurrency-lab` → `lab-03-auth` | `type-system-lab` |
| 全栈开发者 | `lab-02-server-functions` → `lab-03-mini-typescript` | `lab-00-operational-semantics` |
| 类型系统爱好者 | `lab-01-type-inference` → `lab-02-subtyping` | `lab-00-lambda-calculus` |

## 实验环境准备

所有实验均基于现代 Node.js 运行时（22 LTS+）和 TypeScript 5.4+：
...
# 推荐工具链
npm install -g typescript vitest pnpm

# 克隆实验仓库
git clone https://github.com/AdaMartin18010/JavaScriptTypeScript.git
cd JavaScriptTypeScript/website/code-lab
```
