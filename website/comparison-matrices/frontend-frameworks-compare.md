...
Svelte 5（2024-10）是一次根本性重写，Runes 语法取代了经典的 `let` 响应式声明。

| 特性 | 说明 | 状态 |
|------|------|------|
| **Runes** | `$state`、`$derived`、`$effect`、`$props` 等显式响应式原语 | 稳定版 |
| **Snippets** | 替代插槽 (Slots) 的轻量级内容复用机制 | 稳定版 |
| **事件处理** | 移除 `on:` 指令，统一为属性回调 (`onclick`) | 稳定版 |
| **性能基准** | JS Framework Benchmark 中，Svelte 5 在创建行、替换所有行、局部更新测试中均位列第一梯队 | 实测数据 |

**Svelte 5 关键变化**：
- Runes 解决了 Svelte 4 中 `let` 响应式在跨文件和复杂逻辑中的"魔法感"问题，使响应性边界显式可控。
- Snippets 比 Vue/React 的插槽/children 更轻量，无运行时组件开销。
- SvelteKit 2 全面支持 Vite 5，并引入了流式 SSR 与渐进式增强的一等公民支持。
- **性能数据**：Svelte 5 在 JS Framework Benchmark 的"创建 10,000 行"测试中耗时约 250ms，与 Solid 持平，显著优于 React (~450ms) 和 Vue (~400ms)（数据来源：krausest.github.io/js-framework-benchmark，2026-04）。
