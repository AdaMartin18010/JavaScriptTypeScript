...
| 示例主题 | 理论支撑 | 关键概念 |
|---------|---------|---------|
| [Agent 架构设计](./architecture.md) | [并发与异步模型](/concurrency-async/) — 事件循环、异步状态机、背压控制 | Agent 循环本质上是异步生成器（Async Generator）与事件驱动的结合体 |
| [MCP 协议集成](./mcp-integration.md) | [模块系统](/module-system/) — ESM、接口契约、版本兼容性 | MCP 的 Schema 即接口契约，工具发现机制对应动态模块加载 |
| [研究型 Agent 教程](./tutorial.md) | [数据结构与算法](/data-algorithms/) — 向量检索、近似最近邻（ANN）、图遍历 | RAG 的核心是向量空间中的 ANN 搜索；多步推理可建模为图遍历 |
| 工具调用与函数绑定 | [类型系统](/type-system/) — 运行时类型检查、Zod Schema、泛型约束 | Zod Schema 作为 LLM 与 TypeScript 类型系统的桥梁 |
| 记忆管理与上下文压缩 | [执行模型](/execution-model/) — 调用栈、闭包、垃圾回收 | 短期记忆对应调用栈帧；长期记忆的摘要机制类似 GC 中的代际假设 |
