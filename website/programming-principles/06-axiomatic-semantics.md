...
1969 年，Tony Hoare 在《Communications of the ACM》上发表了题为 *An Axiomatic Basis for Computer Programming* 的奠基性论文，提出了后来被称为 **Hoare 逻辑（Hoare Logic）** 的形式化系统。该系统将程序正确性归约为逻辑推理问题：给定前置条件 $P$、程序命令 $C$ 和后置条件 $Q$，如果我们能从公理和推理规则出发证明关系 $\{P\}\, C\, \{Q\}$，那么程序 $C$ 就是（部分）正确的。这一思想革命性地将程序验证从调试和测试的「经验科学」提升为可以严格证明的「演绎科学」。
...
- $P$ 为**前置条件（Precondition）**：程序 $C$ 执行前必须为真的断言；
- $C$ 为**命令（Command）**：一段可执行的程序代码；
- $Q$ 为**后置条件（Postcondition）**：程序 $C$ 执行后（若终止）期望为真的断言。

在**部分正确性（Partial Correctness）**的解释下，$\{P\}\, C\, \{Q\}$ 的含义为：

> 若程序 $C$ 在满足 $P$ 的状态上启动，并且 $C$ 最终终止，则终止后的状态满足 $Q$。

注意部分正确性**不保证终止**——它只关心「如果终止，则结果正确」。若要保证终止，则需要**总正确性（Total Correctity）**，记作 $[P]\, C\, [Q]$，含义为：

> 若 $C$ 在满足 $P$ 的状态上启动，则 $C$ 必然终止，且终止状态满足 $Q$。
...
其中 $Q[e/x]$ 表示将断言 $Q$ 中所有自由出现的 $x$ 替换为表达式 $e$。这条规则是反直觉的：它不是从「赋值前什么为真」推出「赋值后什么为真」，而是从「赋值后我们希望什么为真」反推「赋值前必须满足什么」。这种**向后推理（backward reasoning）**的风格是 Hoare 逻辑的标志。

例如，要证明 `x := x + 1` 将 $x$ 增加 1，我们选择后置条件 $Q = (x = n + 1)$，则前置条件为：
...
两个命令的顺序执行通过中间断言 $R$ 连接。$R$ 既是 $C_1$ 的后置条件，又是 $C_2$ 的前置条件。
...
条件规则要求两个分支在后置条件 $Q$ 上达成一致，无论条件 $b$ 为真或为假。
...
这是 Hoare 逻辑中最深刻的规则。$I$ 称为**循环不变式（Loop Invariant）**——一个在循环体每次执行前后都保持为真的断言。无论循环执行 0 次、1 次还是 $n$ 次，$I$ 始终成立。当循环终止时（$b$ 为假），我们得到 $I \land \neg b$，即循环的最终后置条件。
...
不变式 $I$ 断言：`i` 始终是一个合法的索引（或等于 `n`），且 `s` 已经累加了从 `0` 到 `i-1` 的所有元素。每次迭代后，`i` 增加 1，`s` 增加 `a[i]`，不变式保持成立。当循环终止时，`i = n`，因此 $I \land \neg(i < n)$ 蕴含 $s = \sum_{k=0}^{n-1} a[k]$，即我们想要的后置条件。

要证明**总正确性**，还需证明循环必然终止。这通常通过引入一个**变体函数（Variant Function）**$v$ 实现：

- $v$ 是一个从程序状态到良基集（通常是自然数）的映射；
- 每次循环迭代，$v$ 严格递减；
- 当 $v$ 达到最小值时，循环条件必然为假。

在上述例子中，$v = n - i$ 是一个合适的变体：每次迭代 $i$ 增加 1，因此 $v$ 减少 1；由于 $i \leq n$（由不变式保证），$v \geq 0$。因此循环最多执行 $n$ 次后必然终止。

### 2.4 最弱前置条件（Weakest Precondition）

Edsgar Dijkstra 在《A Discipline of Programming》中提出了**最弱前置条件（Weakest Precondition, WP）**演算，它是 Hoare 逻辑的系统化计算版本。对于给定的命令 $C$ 和后置条件 $Q$，$wp(C, Q)$ 是最大的（最弱的）前置条件集合，使得从该集合中的任何状态启动 $C$，若 $C$ 终止，则终止状态满足 $Q$。

WP 演算的关键在于它是**可计算的（computable）**——对于命令语言的每个构造，存在直接的 WP 公式：

- **Skip**：$wp(\textbf{skip}, Q) = Q$
- **赋值**：$wp(x := e, Q) = Q[e/x]$（与 Hoare 赋值公理一致）
- **顺序**：$wp(C_1; C_2, Q) = wp(C_1, wp(C_2, Q))$
- **条件**：$wp(\textbf{if } b \textbf{ then } C_1 \textbf{ else } C_2, Q) = (b \Rightarrow wp(C_1, Q)) \land (\neg b \Rightarrow wp(C_2, Q))$
- **循环**：$wp(\textbf{while } b \textbf{ do } C, Q)$ 没有闭式解，通常通过寻找不变式 $I$ 使得 $(I \land \neg b) \Rightarrow Q$ 且 $(I \land b) \Rightarrow wp(C, I)$，并以 $I$ 作为前置条件。
...
这里 $x \mapsto 5$ 表示地址 $x$ 存储的值为 5。但如果我们还有一个指向不同地址的指针 $y$，如何表达「$y$ 的内容未被修改」？朴素的 Hoare 逻辑需要引入复杂的「框架条件（frame conditions）」，显式列出所有未被修改的变量和内存位置。

John Reynolds 和 Peter O'Hearn 在 21 世纪初提出的**分离逻辑（Separation Logic）**通过引入新的断言连接词解决了这一问题：

- **分离合取（Separating Conjunction）** $P * Q$：断言堆可以被分割为两个不相交的部分，一部分满足 $P$，另一部分满足 $Q$。
- **分离蕴涵（Separating Implication / Magic Wand）** $P {-\!*} Q$：断言如果我们将一个满足 $P$ 的堆片段连接到当前堆，则合并后的堆满足 $Q$。
- **空堆（Empty Heap）** $\textbf{emp}$：断言堆为空。
- **点断言（Points-to）** $e \mapsto e'$：断言地址 $e$ 恰好存储值 $e'$，且堆只包含这一个单元。
...
框架规则允许我们将验证局部化：如果我们证明了 $C$ 在局部堆 $P$ 上正确运行，那么在更大的堆 $P * R$ 上，$C$ 仍然正确，且未触及的部分 $R$ 保持不变。这极大地简化了对复杂数据结构的验证，如链表、树和图。
...
其中 $\text{ls}(p, \alpha)$ 表示从指针 $p$ 开始、内容为序列 $\alpha$ 的链表。循环将 $x$ 链表的前缀逐步转移到 $y$，不变式精确追踪这种堆结构的演化，而无需提及不相关的内存区域。
...
TypeScript 无法直接表达 $b \neq 0$ 这样的值级约束（除非使用依赖类型或更高级的类型系统），但它通过**控制流分析（Control Flow Analysis, CFA）**提供了近似的保障：
...
> 对于所有变量引用 $x$，$x$ 必须在当前作用域链中被声明。

这相当于在每一条使用变量 $x$ 的命令前插入一个隐式前置条件：$\{ x \in \text{Dom}(\rho) \}$。

另一个例子是 `prefer-const`：

> 若变量 $x$ 在声明后从未被重新赋值，则其声明应使用 `const` 而非 `let`。

这可以被建模为一个关于存储不变式的规则：$\{ \forall C'.\, x \notin \text{Mod}(C') \} \Rightarrow \text{使用 const}$。通过将变量标记为 `const`，我们在语法层面静态保证了赋值不变式，使得后续的任何赋值尝试都会在编译期被拒绝，而非在运行期产生意外行为。
...
React 组件可以被视为一个从「 props + hooks 状态」到「UI 描述」的函数。在每次渲染时，React 按顺序遍历组件函数体，为每个 Hook 分配一个状态槽（state slot）。设组件第 $i$ 次渲染时的 hooks 状态为 $H_i$，则渲染过程 $R$ 满足：

$$
\{ H_{i-1} \}\; R\; \{ H_i \}
$$

Hooks 规则保证了这种映射是**确定性的**：给定相同的前置状态 $H_{i-1}$ 和相同的 props，渲染总是产生相同的 hooks 调用序列，从而每个 hook 总是被分配到相同索引的状态槽。
...
用 Hoare 三元组表达，hooks 规则要求渲染命令 $C$ 满足一个强不变式：
...
关键观察在于：循环不变式 $I$ 巧妙地「记录」了已经完成的乘法工作量。变量 `z` 始终等于 $m$ 乘以已经减去的 `y` 的数量。当 `y` 递减至 0 时，`z` 自然等于 $m \cdot n$。这种「 bookkeeping 」思维——通过不变式在循环迭代间维持某种累积量——是编写和验证循环程序的核心技能。
...
框架规则的核心洞见是**局部推理（local reasoning）**：我们只需要关注当前操作所触及的内存片段（$x \mapsto 5$），而将所有不相关的状态打包进框架 $R$。验证工具（如 Infer）能够自动推断 $R$，从而将复杂的全局堆验证分解为可管理的局部问题。这与现代软件工程中「关注点分离（separation of concerns）」的设计哲学高度一致。
...
5. **分离逻辑通过分离合取和框架规则实现了堆的局部推理**：传统 Hoare 逻辑在处理指针和堆时面临框架条件的组合爆炸，分离逻辑通过 $*$ 连接词和框架规则，使得验证可以聚焦于当前操作的局部内存 footprint。
