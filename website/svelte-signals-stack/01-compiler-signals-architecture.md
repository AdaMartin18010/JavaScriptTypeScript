...
> 下图展示了 Svelte 编译器从源代码到优化 JavaScript 的完整处理流程，以及 Signal 依赖图的构建和虚拟 DOM 的消除过程。
...
**流程解读**：Svelte 编译器采用经典的多阶段流水线设计。在**分析阶段**提取 Signal 依赖图，这是 Compiler-Based 响应式的核心——所有依赖关系在编译时即被确定；在**转换阶段**直接消除虚拟 DOM，将模板编译为原生 DOM 操作指令。这种"编译时决策"模式使得运行时无需维护 VDOM 树或执行 Diff 算法，是实现零运行时框架开销的关键。
...
- **架构优势**：无虚拟 DOM、无 Hook 规则限制、自动内存管理，使性能接近原生 JavaScript。
- **实践建议**：理解编译器输出有助于调试性能问题，建议结合 JS Framework Benchmark 评估实际场景。

> 💡 **相关阅读**: [响应式系统深度原理](14-reactivity-deep-dive) · [Svelte 5 Runes 完全指南](02-svelte-5-runes)
