...
这是 Svelte 5 相对 Svelte 4 的重大改进。在 Svelte 4 中，数组方法（`push`、`pop`、`splice` 等）不会触发更新，必须赋值新数组。Svelte 5 的 Proxy 拦截了这些方法：
...
`$derived` 用于创建基于其他响应式值的纯计算属性。它自动追踪依赖，并在依赖变化时重新计算。
...
`$effect` 用于处理副作用，如 DOM 操作、网络请求、订阅外部数据源等。它在组件挂载时运行，并在依赖变化时重新运行。
