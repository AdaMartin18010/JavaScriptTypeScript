...
### sv CLI 与社区 Add-ons（2026-03+）

`sv` CLI 已从单纯的项目脚手架演进为**生态集成工具**，支持通过 add-ons 一键集成社区方案：

```bash
# 查看可用 add-ons
npx sv add --help

# 一键添加认证（better-auth 官方集成）
npx sv add auth

# 一键添加数据库 ORM（Drizzle 或 Prisma）
npx sv add drizzle

# 一键添加 UI 组件库（Skeleton、Shadcn-svelte 等）
npx sv add ui --library skeleton

# 一键添加测试（Playwright + Vitest）
npx sv add testing
```

**Add-on 工作原理**: `sv add` 读取社区维护的 `svelte-add` manifest，自动执行：

1. 安装 npm 依赖
2. 修改 `svelte.config.js` / `vite.config.ts`
3. 生成样板代码（如 `src/lib/auth.ts`）
4. 输出下一步配置指引

> 来源: [sv CLI 文档](https://svelte.dev/docs/cli)、[svelte-add 仓库](https://github.com/svelte-add/svelte-add)

> 下图展示了 SvelteKit 处理一次完整页面请求的生命周期，从客户端 HTTP 请求到最终 HTML 响应的完整数据流。
...
sequenceDiagram
    ...
    participant S as +page.svelte
    participant A as +server.ts
    C->>R: HTTP Request
    R->>L: 匹配路由并调用load
    L->>A: 可选API请求
    A-->>L: JSON数据
...
**生命周期解读**：SvelteKit 的请求处理遵循清晰的分层模型。Router 负责路由匹配和调度；`+page.ts` 中的 `load()` 函数是同构数据获取的核心，可在服务端和客户端复用；`+server.ts` 提供独立的 API 端点能力。这种设计使得数据流单向可追溯，SSR 与 CSR 之间的切换对开发者透明，是实现渐进式增强和流式渲染的基础架构。
...
### 错误边界（Error Boundaries）— 服务端错误隔离（5.53+ / Kit 2.54+）

SvelteKit 2.54 引入了 **server-side error boundaries**，允许在 `+error.ts` 中定义错误恢复逻辑，将服务端错误隔离在特定路由范围内，防止整个页面崩溃。

```ts
// src/routes/dashboard/+error.ts
...
// 错误边界配置：定义哪些错误可被捕获、如何降级
export const handleError = ({ error, status }) => {
  if (status === 503) {
    // 服务暂时不可用：返回降级数据，页面仍可渲染骨架屏
    return { fallback: { data: [], status: 'degraded' } };
  }
  // 其他错误继续向上抛
  throw error;
};
...
<script>
  ...
  let { status, error, fallback } = $derived(page);
</script>

{#if fallback}
  <!-- 降级模式：用缓存/骨架数据继续渲染 -->
  <DashboardShell data={fallback.data} mode="offline" />
{:else}
...
**与 `hooks.ts` 全局错误处理的对比**：

| 维度 | `hooks.ts` `handleError` | `+error.ts` 错误边界 |
|:---|:---|:---|
| 作用范围 | 全局 | 路由局部 |
| 能否降级渲染 | ❌ 只能返回错误页 | ✅ 可返回 fallback 数据继续渲染 |
| 刷新粒度 | 整页刷新 | `depends()` 细粒度重取 |
| 适用场景 | 未预期异常、日志记录 | 预期可能失败的依赖（第三方 API） |

> 来源: [SvelteKit Error Boundaries RFC](https://github.com/sveltejs/kit/discussions)、Svelte 5.53 Release Notes
...
| **TypeScript 支持** | 优秀（自动生成 `$types`，兼容 TS 6.0） | 优秀 | 优秀 |
...
| **TypeScript 6.0 支持** | 2.56+ | 完整兼容 TS 6.0 的 `using` 声明、装饰器元数据、`Array.prototype.toSorted` 类型推断等新特性 |
| **Server-side Error Boundaries** | 2.54+ | `+error.ts` 路由级错误隔离，支持 fallback 降级渲染 |
| **`sv add` 社区 Add-ons** | sv 0.14+ | 官方 CLI 支持一键集成 Drizzle、better-auth、Skeleton 等社区方案 |
...
#### Remote Functions API 演进（Kit 2.56–2.59）

SvelteKit 2.56–2.59 对 Remote Functions 进行了多次迭代，以下是最新稳定语义（截至 2.59.x）：
...
export const listUsers = remote(
  async ({ requested }, page: number, limit: number) => {
    // requested: 客户端实际请求的信号，可读取缓存策略和依赖标记
  ...
  },
  {
    ...
    maxAge: 5,
    // 单次请求最多返回 100 条
    requested: { maxLimit: 100 }
  }
);
```

**`refresh` 返回 Promise（2.57+）**:

```svelte
<script>
  import { listUsers } from '$lib/server/remote';

  let page = $state(1);
  let usersQuery = $derived(listUsers(page, 20));

  // usersQuery.refresh() 返回 Promise，可 await 刷新完成
...
<button onclick={refreshUsers}>手动刷新</button>
```
...
```ts
// 非响应式场景：一次性调用，不走缓存
const result = await listUsers.query.run(1, 20);
```

**`query.live` 实验性实时订阅（2.59+）**:

```svelte
<script>
  import { liveUserCount } from '$lib/server/remote';

  // 实验性：建立 Server-Sent Events 连接，服务端推送更新
  let count = $state(0);
  $effect(() => {
    const unsubscribe = liveUserCount.query.live((value) => {
      count = value;
    });
    return unsubscribe;
  });
...
> ⚠️ **实验性警告**: `query.live` 依赖 SSE 基础设施，API 在 2.59 中标记为 `experimental`，后续版本可能变更。
...
- **文件系统路由**：SvelteKit 基于目录结构自动生成路由，支持布局嵌套、动态参数和路由守卫。
- **全栈数据流**：`+page.server.ts` 的 `load` 函数与 `actions` 实现了类型安全的服务端渲染和表单处理。
- **Hooks 中间件**：`handle` 序列允许组合认证、日志、CORS 等横切关注点，支持 TypeScript 类型扩展。
- **适配器部署**：通过适配器抽象，同一套代码可部署到 Node.js、Cloudflare Workers、Vercel Edge 等多种平台。
- **实践建议**：充分利用自动生成的 `$types` 导入，始终为 `locals` 和 `Platform` 扩展类型声明。

> 💡 **相关阅读**: [边缘同构运行时](06-edge-isomorphic-runtime) · [生产实践指南](08-production-practices)
...
> **最后更新**: 2026-05-06 | **SvelteKit 版本**: 2.59.x | **数据来源**: kit.svelte.dev, GitHub Stars 2026-05, npm registry 2026-05
