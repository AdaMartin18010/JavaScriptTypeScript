...
> 下图展示了 `.svelte` 文件在 TypeScript 类型检查管道中的完整流转过程。

```mermaid
flowchart TD
    A[.svelte文件] --> B[svelte2tsx转换]
    B --> C[TypeScript类型检查]
    C --> D{类型错误?}
    D -->|是| E[生成诊断信息]
    D -->|否| F[编译通过]
...
**类型检查流程解读**：Svelte 的类型检查并非原生 TypeScript 直接处理，而是通过 `svelte2tsx` 先将 `.svelte` 文件转换为等效的 `.tsx` 表示，再交由 TypeScript 编译器进行标准类型分析。这种桥接设计让模板中的表达式、Props 绑定和事件处理都能获得完整的类型推断。当类型错误被检测到时，诊断信息会同时反馈到 IDE（通过 Language Server）和 CI 流水线（通过 `svelte-check` CLI），形成开发时与构建时的双重类型保障。
...
- **自动类型生成**：SvelteKit 的 `$types` 模块为 `load`、`actions`、`params` 提供精确的类型推导。
- **事件与上下文类型化**：使用原生 DOM 事件类型和 `Symbol` 键的 `setContext`/`getContext` 避免隐式 `any`。
- **常见陷阱**：避免直接解构 `$state` 对象、注意 `$effect` 的条件分支依赖追踪，并启用 `strict: true`。
- **实践建议**：结合 `svelte-check` 和 `import type` 规范，在 CI 中运行类型检查以防止运行时错误。

> 💡 **相关阅读**: [Svelte 语言完全参考](12-svelte-language-complete) · [Svelte 5 Runes 完全指南](02-svelte-5-runes)
