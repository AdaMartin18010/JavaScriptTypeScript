...
keywords: 'Svelte语法, 语义模型, 指令, Runes公理, Store, 反例, 语言参考'
---
...
- Svelte 5 Runes（$state、$derived、$effect、$props）是现代 Svelte 响应式编程的核心
- 编译器在构建阶段将模板和指令转换为高效的原生 DOM 操作，无需虚拟 DOM 运行时
- `.svelte.ts` 文件让响应式逻辑可复用于组件外部，实现真正的模块化状态管理
- Store（writable、readable、derived）与 Runes 可共存，根据场景选择合适抽象层级
- 掌握 Svelte 指令（bind、on、use、transition）和生命周期是高效开发的必备基础

## 参考资源

- 🌐 [Svelte 官方文档](https://svelte.dev/docs) — 最全面的 API 参考和教程
- 🎮 [Svelte REPL](https://svelte.dev/playground) — 官方交互式 playground，实时验证代码
- 🔧 [Svelte DevTools](https://github.com/sveltejs/svelte-devtools) — 浏览器扩展，调试组件树与状态
- 📦 [SvelteKit 文档](https://kit.svelte.dev/docs) — 全栈框架的路由、表单和服务端能力
- 💬 [Svelte Discord](https://svelte.dev/chat) — 活跃社区，提问和学习资源分享
...
**任务**: 下面的代码包含 3 个 Svelte 语法/语义层面的问题。找出所有问题并给出修复后的代码。
...
**预期行为**: 点击 "Next" 按钮时，段落应依次显示 'a' → 'b' → 'c'，同时页面标题同步更新。

**常见错误** ⚠️:
> 认为 `$state(items[index])` 会建立 `items` 和 `index` 的双重依赖。实际上 `$state` 只接收初始值，不会追踪表达式中的变量。应使用 `$derived` 来声明基于其他响应式值的计算。此外，在 `$effect` 中同步修改被追踪的 `$state` 会导致无限循环。
...
### 🛠️ Try It: 推导动态依赖的变更传播

**任务**: 分析以下代码的依赖图，并预测当点击 "Switch Mode" 后，再点击 "Increment A" 时控制台会输出什么。
...
<script>
  ...
  let display = $derived(mode === 'a' ? a : b);
...
<p>{display}</p>
<button onclick={() => mode = mode === 'a' ? 'b' : 'a'}>Switch Mode</button>
...
1. 初始时 `display` 依赖哪些 Source？
2. 点击 "Switch Mode" 后，依赖图如何变化？
3. 切换后点击 "Increment A"，`$effect` 会触发吗？为什么？

**预期行为**: 理解 Svelte 5 的动态依赖清理机制——每次 `$derived` 重新执行时，旧的依赖关系会被清除，新的依赖关系根据当前执行路径重新建立。

**常见错误** ⚠️:
> 认为 `$derived` 的依赖是静态的、累加的。实际上 Svelte 5 在每次重新执行 `$derived`/`$effect` 前会**清理所有旧依赖**，然后根据本次执行路径建立**全新的依赖图**。这是与 Vue Watcher 的关键差异，也是避免"死依赖"问题的设计。

**验证方式**:

- [ ] 正确画出初始依赖图
- [ ] 正确预测 Switch Mode 后的依赖图
- [ ] 能解释为什么 Increment A 在 mode='b' 时不触发 effect
- [ ] 在 Svelte REPL 中验证预测
...
### 🛠️ Try It: 编写一个类型安全的泛型表格组件

**任务**: 使用 `<script generics="T">` 编写一个泛型 `DataTable` 组件，支持任意数据类型，同时保证 TypeScript 类型推导。
...
<script lang="ts">
  ...
  let { data, columns, getRowId }: Props = $props();
</script>
...
- TypeScript 能推断 `T` 为 `{ id: string; name: string; age: number }`
- `columns` 的 `key` 只能是 `name` 或 `age`，其他值报类型错误
- 传入错误类型的 `data` 报类型错误

**常见错误** ⚠️:
> 忘记在 `<script>` 标签上声明 `generics="T"`，导致 TypeScript 将 `T` 视为未定义变量。或者将 `generics` 属性写在 `<script lang="ts">` 之后单独一行，正确的语法是 `<script lang="ts" generics="T extends Record<string, any>">`。

**验证方式**:

- [ ] 组件能正确渲染表格
- [ ] `columns` 中有不存在的 key 时 TypeScript 报错
- [ ] `data` 传入非数组时 TypeScript 报错
- [ ] `getRowId` 参数类型正确推导
...
- [Svelte REPL](https://svelte.dev/playground) — 官方交互式 playground
- [Svelte DevTools](https://github.com/sveltejs/svelte-devtools) — 浏览器调试扩展
- [Svelte Check](https://www.npmjs.com/package/svelte-check) — 命令行类型检查
...
> 💡 **相关阅读**: [Svelte 5 Runes 完全指南](02-svelte-5-runes) · [组件开发模式大全](13-component-patterns)
