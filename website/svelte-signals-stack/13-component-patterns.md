...
### 🛠️ Try It: 用 Snippet 重构一个基于 Slot 的 Card 组件

**任务**: 将下面的 Svelte 4 风格 Slot-based Card 组件重构为 Svelte 5 的 Snippet-based 版本，并增加参数传递能力。

**starter code** (Svelte 4 版本):
...
1. 新版应支持 `header`、`children`、`footer` 三个 Snippet
2. `header` Snippet 接收 `{ title: string }` 参数
3. `footer` 为可选 Snippet，不存在时不渲染 footer 区域
4. 提供完整 TypeScript 类型
...
**常见错误** ⚠️:
> 将 Snippet 当作普通 prop 使用时忘记调用（如 `{header}` 而非 `{@render header()}`）。Snippet 是编译后的函数，必须使用 `{@render}` 指令调用。另外，给 Snippet 参数定义类型时，应使用 `Snippet<[ParamType]>` 而非普通函数类型。

**验证方式**:

- [ ] 新版 Card 正确渲染三个区域
- [ ] header 能接收并使用参数
- [ ] 不传 footer 时不渲染 footer 区域
- [ ] TypeScript 对 Snippet 参数类型正确报错/推导
- [ ] 支持默认内容（header 不传时显示默认标题）
...
### 🛠️ Try It: 实现一个带虚拟列表的 Select 组件

**任务**: 基于组件中的 `VirtualList` 模式，实现一个能高效渲染 10,000 个选项的下拉选择器 `VirtualSelect`。要求：
...
<div class="virtual-select" bind:clientHeight={containerHeight}>
  <!-- 实现虚拟列表和键盘事件... -->
</div>
```
...
- 10,000 个选项流畅滚动，不卡顿
- 按 ↑/↓ 键时选中项移动，且超出可视区时自动滚动
- 按 Enter 触发 `onSelect`
- 初始选中 `value` 对应的选项

**常见错误** ⚠️:
> 在虚拟列表中使用数组索引作为 `{#each}` 的 key（如 `(index)`）。当列表过滤或排序时，索引 key 会导致 DOM 复用错误、状态错乱。虚拟列表应使用选项的唯一标识（如值本身）作为 key。

**验证方式**:

- [ ] 10,000 选项列表滚动流畅（60fps）
- [ ] 键盘导航正常工作
- [ ] 选中项始终可见
- [ ] 点击选项能正确选择
- [ ] 长列表内存占用稳定
