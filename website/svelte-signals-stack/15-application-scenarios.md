...
>
> 本指南基于 Svelte 5 与 SvelteKit 2.x 的技术特性，结合生产环境实践经验，提供系统化的决策框架。
> 内容涵盖项目类型匹配、团队规模适配、性能要求分析、垂直行业案例、项目规模规划以及常见反模式警示，帮助技术团队做出理性、可落地的技术选型决策。
...
这是 Svelte 最容易被低估的场景。
当需要在第三方页面（如 WordPress、Shopify 主题、其他框架构建的站点）中嵌入交互组件时，Svelte 编译为 Web Component 的能力使其成为最优选择：
...
小型团队的核心诉求是 **开发速度和认知负载**。
Svelte 的模板语法减少了 30-50% 的样板代码：
...
Svelte 的首屏优势不仅来自运行时小，更来自其 **编译时树摇（Tree-shaking）** 的高效性。
未使用的过渡效果、动画辅助函数、内置指令在编译阶段即被剔除，不会进入 bundle。
...
Svelte 5 的 Runes 系统通过编译时将响应式依赖转换为细粒度的订阅关系。
一个独立状态的更新路径是：
...
理性选型的另一半是清楚知道什么场景不适合。
以下分析基于当前（2026年初）的生态系统成熟度、人才市场现状和技术架构匹配度。
...
Svelte 的 Store（`writable` / `readable` / `derived`）适合管理应用级数据，但不适合表达复杂的有限状态机（FSM）。
虽然可以手动实现状态模式，但失去了可视化调试和状态图生成的重要价值。
...
**任务**: 根据以下项目描述，使用本文档的决策矩阵和流程图，做出技术选型决定并说明理由。

**项目 A**: 一个 5 人团队的内部数据仪表盘，需要实时显示后端推送的指标数据，不需要 SEO，不需要移动端适配。

**项目 B**: 一个面向消费者的电商网站，需要良好的 SEO、购物车、用户账户、支付集成，团队 15 人，预算有限。

**预期产出**: 对两个项目分别输出：

1. 推荐的技术栈（Svelte 版本 + 后端 + 数据库 + 部署平台）
2. 使用本文档中的决策流程图说明每一步的判断
3. 至少一个"不适用场景"的引用（为什么某些技术被排除）

**常见错误** ⚠️:
> 只根据"Svelte 性能好"就推荐所有项目都用 Svelte。实际上，选型应综合考虑团队规模、生态需求、SEO 要求和维护成本。本文档明确指出了 Svelte 不适用的场景。

**验证方式**:

- [ ] 每个决策都有文档中的具体依据
- [ ] 至少引用了一个反模式（"什么时候不该用 Svelte"）
- [ ] 为项目 B 的支付集成说明了第三方库选择策略
