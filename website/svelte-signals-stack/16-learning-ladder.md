...
以下学习进度图将 12 周的学习路径划分为四个递进阶段，每阶段聚焦不同的能力维度。这种块状推进的方式既保证了知识体系的完整性，又避免了过早引入超出当前水平的复杂概念。
...
flowchart LR
    ...
    subgraph Week3-4["第3-4周: 进阶"]
        B1[Store模式] --> B2[高级Runes]
    end
    subgraph Week5-8["第5-8周: 框架"]
        C1[SvelteKit路由] --> C2[数据加载]
    end
    subgraph Week9-12["第9-12周: 生产"]
        D1[SSR/Edge] --> D2[测试部署]
    end
    Week1-2 --> Week3-4 --> Week5-8 --> Week9-12
```

每个阶段的学习产出都具有明确的可交付成果：第 2 周结束时应能独立编写交互组件，第 8 周结束时应具备全栈开发能力，第 12 周结束时应能独立完成项目的测试与部署。建议严格按照阶段边界验收，不要急于跳过基础内容。
...
- 100 天学习阶梯从 Level 0 预备知识到 Level 8 开源贡献，覆盖从入门到精通的完整路径
- 每日"理论+实践+复盘"的节奏是建立 Svelte 响应式肌肉记忆的关键
- 学习应遵循渐进原则：Level 1-3 先让功能跑通，Level 6+ 再系统优化性能
- 培养阅读源码、跟踪 RFC、参与社区的习惯，比记住任何 API 都更重要

> 💡 **相关阅读**: [知识图谱与思维工具](17-knowledge-graph) · [Svelte 5 Runes 完全指南](02-svelte-5-runes)

## 参考资源

- 📘 [Svelte 官方 Interactive Tutorial](https://svelte.dev/tutorial)
- 🏠 [SvelteKit 官方文档](https://kit.svelte.dev/docs/introduction)
- 💬 [Svelte Discord 社区](https://svelte.dev/chat)
- 🧪 [Svelte REPL 在线演练场](https://svelte.dev/repl)
- 📚 [Svelte Society 学习资源](https://sveltesociety.dev/resources)
