...
| **SvelteKit** | 2.59.1 | 2026-05-01 | [全栈框架指南](03-sveltekit-fullstack) | ✅ 稳定 |
...
| **TC39 Signals** | Stage 1 | 2025-06 | [路线图](11-roadmap-2027) | ⏸️ 停滞（2025-08 后零 commit，无 Stage 2 计划）|

### 安全漏洞追踪

> 生产环境安全基线: Svelte ≥ 5.53.5 / devalue ≥ 5.6.4

| CVE 编号 | 发布时间 | 影响版本 | 修复版本 | 严重程度 | 状态 |
|:---|:---|:---|:---|:---:|:---|
| CVE-2026-27125 | 2026-02 | < 5.51.5 | ≥ 5.51.5 | 🔴 High | ✅ 已修复 |
| CVE-2026-27119 | 2026-02 | < 5.51.5 | ≥ 5.51.5 | 🔴 High | ✅ 已修复 |
| CVE-2026-27121 | 2026-02 | < 5.51.5 | ≥ 5.51.5 | 🟠 Medium | ✅ 已修复 |
| CVE-2026-27122 | 2026-02 | < 5.51.5 | ≥ 5.51.5 | 🟠 Medium | ✅ 已修复 |
| CVE-2026-27901 | 2026-03 | < 5.53.5 | ≥ 5.53.5 | 🟠 Medium | ✅ 已修复 |
| CVE-2026-27902 | 2026-03 | < 5.53.5 | ≥ 5.53.5 | 🟡 Low | ✅ 已修复 |
| CVE-2026-30226 | 2026-03 | devalue < 5.6.4 | ≥ 5.6.4 | 🔴 High | ✅ 已修复 |

> 来源: [Svelte GitHub Security Advisories](https://github.com/sveltejs/svelte/security/advisories)、[NVD](https://nvd.nist.gov/)
...
| 指标 | 数据 |
|------|------|
| **当前阶段** | Stage 1（提案自评 "Stage 0" 成熟度） |
| **GitHub Stars** | 4,100+ |
| **Open Issues** | 120+（含 2026 年新增算法争议） |
| **最后 commit** | 2025-08-11（`9124ed91`）|
| **2026 年 commit 活动** | **零 commit**（功能冻结审查期） |
| **提案链接** | [tc39/proposal-signals](https://github.com/tc39/proposal-signals) |

### Stage 2 推进状态（2026-05 更新）

TC39 Signals **未进入 Stage 2 推进轨道**。README 明确列出三条前置要求，且 2026 年 5 月 TC39 会议（阿姆斯特丹）议程未包含 Signals 议题：

1. 开发多个生产级、高性能 polyfill 实现
2. 将 Signal API 集成到大量 JS 框架中
3. 对 API 扩展空间有充分理解
...
|:---|:---|:---|:---|
| **#278** | 2026-04-18 | Unwatched Computed 的脏传播：无 sink 的 State 变更时，Computed 是否应在下次 `.get()` 重新求值？ | Svelte 通过版本号系统已解决，不受此争议影响 |
| **#279/#280** | 2026-04-22 | `equals` 回调中的依赖追踪顺序："recalculate" 算法中 `computing` 恢复与 "set Signal value" 调用的顺序 | Svelte 的 `safe_equals` 在 `active_reaction` 恢复后执行，无此问题 |
| **#282** | 2026-05-01 | `Watcher` 的 `~pending~` 状态是否必要？polyfill 实际上跳过该状态 | Svelte 无直接等价状态，标准简化有利 |

> 详细分析见 [21-tc39-signals-alignment](21-tc39-signals-alignment.md) 第 3.4 节。
...
| 调度 | 内置 Batch 微任务批量更新 | 无内置调度，`notify` 同步调用 |
| 互操作 | 框架内部实现 | 跨框架标准（长期目标） |
| 性能 | 编译器优化 + 版本号系统 | 依赖 JS 引擎实现 |
| 成熟度 | 生产级（5.55.5），边界情况已处理 | 算法规范存在已知争议 |

**Svelte 团队立场（Rich Harris, TC39 第 104 次会议）**：观望至 Stage 3，不早于 Stage 2.7 评估编译器生成原生 Signal API 的可行性。这一策略在 2026 年的算法争议背景下显得尤为审慎。

---

## 前沿 API 追踪

### Svelte 5.60+ 预览特性（基于 main 分支和 RFC 草案）

| API / 特性 | 状态 | 预计版本 | 说明 |
|:---|:---|:---:|:---|
| **Compiler IR** | RFC 讨论中 | 5.60+ / 6.0 | 将 AST 转换为中间表示，支持 WASM/Native 后端编译。Rich Harris 2026-04 公开线程引发广泛讨论 |
| **`$state.raw` 增强** | 实验性 | 5.56 | 深层响应式的替代方案，针对大型表格和 Canvas 数据的性能优化 |
| **`svelte-migrate` 自动化** | Beta | 5.53+ | Svelte 4 → 5 自动代码转换工具，社区已有 200+ 项目成功迁移 |
| **`hydrate: false`** | 实验性 | 5.54+ | 完全静态渲染选项，纯静态站点构建体积进一步压缩 |
| **`$effect.active`** | 稳定 | 5.55.0 | 检测当前是否在 effect 上下文的运行时 API |
| **`$props.id()`** | 稳定 | 5.51.0 | 生成 SSR 友好的稳定 ID，解决 SSR/CSR ID 不一致问题 |
| **`--generate-stats`** | 稳定 | 5.55.0 | 编译器构建分析数据输出标志 |
| **`warningFilter`** | 稳定 | 5.53.0 | 项目级抑制特定编译器警告 |

### SvelteKit 3.0 前瞻（基于 RFC 和里程碑）

| 特性 | 状态 | 预计时间 | 说明 |
|:---|:---|:---:|:---|
| **Vite 6 Environment API 全面适配** | 开发中 | 2026 H2 | 同时构建 client/server/edge 多环境 |
| **Rolldown 默认构建后端** | 等待 Vite 7 | 2026 H2 / 2027 | 构建速度提升 3-5 倍 |
| **原生 WebSocket 支持** | 2.56.0 已发布 | 已可用 | `+server.ts` 中处理 WebSocket 升级 |
| **Streaming SSR 增强** | 2.58.0 实验性 | 持续迭代 | SSE 开箱即用，对标 Next.js Streaming |
| **条件 SSR** | 2.57.0 已发布 | 已可用 | 根据请求头动态决定是否服务端渲染 |
