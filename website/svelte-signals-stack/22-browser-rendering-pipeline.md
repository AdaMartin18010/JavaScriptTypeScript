...
> **分析基准**: Chrome 124+ (Blink Rendering NG) / Firefox 125+ (Servo/Wl) / Safari 17+ (WebKit)
> **Svelte 版本**: 5.55.5
...
- React 19 创建 10,000 行：~450ms
- 差异根源：Svelte 的 `cloneNode` 批量创建 + 直接属性赋值，React 的 `createElement` + 属性对象遍历 + `appendChild`
...
如果必须在 effect 中执行长任务，使用 `scheduler.yield()`（Chrome 124+）：
...
| 对象类型 | Svelte 5 | React 19 | 差异原因 |
|:---|:---:|:---:|:---|
| 组件实例 | 无（纯函数） | Fiber 节点 | Svelte 编译为函数 |
| 状态容器 | Source 对象 (~40 bytes) | useState Hook (~80 bytes) | Svelte 更轻量 |
| VDOM 树 | 无 | 整棵树 | Svelte 直接操作 DOM |
| Effect 链表 | 数组引用 | 环形链表 | 实现差异 |
| **总计 (10k 组件)** | **~18MB** | **~42MB** | Svelte 节省 57% |

---

> 附录更新: 2026-05-06 | 浏览器对齐: Chrome 124+ / Firefox 125+ / Safari 17+ | 源码对齐: Svelte 5.55.5
...
| 场景 | Svelte 5 | React 19 | 差异原因 |
|:---|:---:|:---:|:---|
| 单次 state 更新 | ~0.1ms JS | ~2.5ms JS | 无 VDOM diff |
| 1000 行列表更新 | ~12ms 总计 | ~45ms 总计 | O(1) vs O(n) |
| 初始渲染 | ~250ms | ~450ms | 更少运行时开销 |
| 内存占用 (10k 组件) | ~18MB | ~42MB | 无 VDOM 树 |
| Bundle 体积 (Hello World) | ~2KB | ~45KB | 编译器优化 |

> 数据来源: JS Framework Benchmark 2026-04, 实验室环境 Chrome 124, M3 MacBook Pro
...
> 跨浏览器附录更新: 2026-05-06 | 浏览器对齐: Chrome 124+ / Firefox 125+ / Safari 17+
...
> 优化检查清单更新: 2026-05-06 | 浏览器对齐: Chrome 124+ / Firefox 125+ / Safari 17+ | Svelte 对齐: 5.55.5
...
> 网络与渲染附录更新: 2026-05-06 | 浏览器对齐: Chrome 124+ / Firefox 125+ / Safari 17+ | SvelteKit 对齐: 2.59.x
...
> 场景与移动端附录更新: 2026-05-06 | 浏览器对齐: Chrome 124+ / Firefox 125+ / Safari 17+ | 移动端: iOS 17+ / Android 14+
