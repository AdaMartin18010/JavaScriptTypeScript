...
5. **Edge 环境重塑 RPC 选择**：CPU 时间限制（Cloudflare Workers 50ms/10ms）、无 Node.js 模块（不能用 grpc-js）、HTTP/2 支持有限，使得 Connect Protocol (JSON) 和 tRPC Batch Link 成为 Edge 函数间通信的最优解。SSE 是 WebSocket 在 Edge 中的有效替代。
...
**传输协议与批处理**：
tRPC 默认使用 HTTP/1.1 JSON，但支持多种传输层优化：
...
### Connect RPC：Protobuf 与 HTTP 的融合

Connect RPC 独特之处在于同时支持三种传输协议，使服务端可以同时服务不同类型的客户端：
