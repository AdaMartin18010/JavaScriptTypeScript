---
title: 'RPC 框架与类型安全传输'
description: 'RPC Frameworks and Type-Safe Transport: tRPC v11, Connect 2.1.1, Hono RPC, oRPC, gRPC-Web, JSON-RPC 2.0, Schema Evolution'
---
...
2. **tRPC v11 是全栈 TypeScript 的默认选择（2026）**：v11 深度集成 TanStack Query v5，实现缓存策略与失效逻辑的原生互通；同时提供 React Server Components（RSC）原生支持，Server Component 可直接调用 tRPC 路由而无需客户端 JavaScript 开销。

3. **Connect 2.1.1 是三协议兼容的桥梁**：同时支持 Connect Protocol（浏览器原生）、gRPC（数据中心标准）和 gRPC-Web（现有客户端兼容），是唯一能在浏览器直接运行且兼容数据中心 gRPC 的框架，无需 Envoy 代理。

4. **Schema 演化是工程必需品**：Protobuf 有严格的向后兼容性规则（安全：添加新字段；危险：修改编号/类型）；tRPC 依赖 Zod schema 的谨慎扩展。没有规划的类型变更等于 API 破坏，版本前缀和转换管道是最佳实践。

5. **Edge 环境重塑 RPC 选择**：CPU 时间限制（Cloudflare Workers 50ms/10ms）、无 Node.js 模块（不能用 grpc-js）、HTTP/2 支持有限，使得 Hono RPC（`hc` 零开销客户端）和 Connect Protocol (JSON) 成为 Edge 函数间通信的最优解。SSE 是 WebSocket 在 Edge 中的有效替代。

6. **oRPC 填补了类型安全与 OpenAPI 文档之间的空白**：自动生成 OpenAPI 3.1 输出，支持 Zod/Valibot/ArkType，适合需要对外暴露 API 的边缘服务，是比「tRPC + 第三方 OpenAPI 生成器」更简洁的选择。
...
**Level 2 — 路由级类型推断**：
路由器定义中嵌入类型，客户端根据路由自动推断参数和返回类型。tRPC v11、Hono RPC 和 Connect 属于这一层。服务端修改路由结构后，客户端的调用代码会立即出现类型错误。
...
### Hono RPC：边缘原生的零开销客户端

Hono RPC 通过 `hc`（Hono Client）实现端到端类型推断，在边缘环境中具有独特优势：

- **零运行时开销**：`hc` 仅在编译时提供类型，运行时是普通 `fetch` 调用，bundle 大小接近零
- **Edge Runtime 原生**：无额外依赖，支持 Cloudflare Workers、Deno Deploy、Vercel Edge、Bun
- **生态规模**：Hono 达到 **2.8M 周下载量**（2026），是轻量级 Edge API 的首选
...
oRPC 是 2025–2026 年崛起的 RPC 框架，设计目标是**同时提供端到端类型安全与自动 OpenAPI 文档生成**。

- **OpenAPI 3.1 自动生成**：schema 变更自动同步，无需手动维护 Swagger
...
**与 tRPC 的对比**：tRPC v11 专注全栈 TypeScript 体验；oRPC 当你**既需要类型安全又需要 OpenAPI 文档**时的最佳选择，适合对外暴露 API 的场景。

### Connect RPC 2.1.1：Protobuf 与 HTTP 的融合

Connect RPC（当前稳定版 **2.1.1**，2026）独特之处在于同时支持三种传输协议，使服务端可以同时服务不同类型的客户端：
...
- 启用 tRPC v11 的 HTTP Batch Link，减少请求数
...
**选择指南**：公共 API 暴露优先 REST + OpenAPI；全栈 TypeScript 单体优先 tRPC v11；多语言微服务优先 Connect 2.1.1 + Protobuf；复杂数据图查询需求优先 GraphQL。
...
| 多语言微服务 | **Connect 2.1.1 + Protobuf** | Protobuf 跨语言，gRPC/Connect/Web 三协议兼容 | 需要 .proto 文件管理流程 |
...
| 第三方 API 暴露 | **oRPC** | 类型安全 + 自动 OpenAPI 3.1 输出 | 新兴框架，社区资源较少 |
