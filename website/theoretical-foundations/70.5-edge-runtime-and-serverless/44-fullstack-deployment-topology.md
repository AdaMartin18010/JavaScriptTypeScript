...
Docker 多阶段构建的关键是**层缓存策略**：依赖文件（`package.json`、`pnpm-lock.yaml`）变更频率低，应置于早期层；业务代码变更频率高，置于后期层。这样代码更新时无需重新安装依赖。`node:alpine` 镜像将基础体积从 1GB+ 缩减到 ~200MB，配合 `pnpm deploy` 仅复制生产依赖可进一步瘦身。
...
| 大型 Monorepo | Turborepo + Vercel / Nx + AWS | 增量构建，远程缓存 | 配置复杂，学习曲线陡 |
